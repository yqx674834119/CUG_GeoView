...
def _custom_fwd_compat(**kwargs):
    """Adapt AMP decorator kwargs across different torch versions."""
    supported = inspect.signature(custom_fwd).parameters
    return custom_fwd(**{key: value for key, value in kwargs.items() if key in supported})


def _custom_bwd_compat(**kwargs):
    supported = inspect.signature(custom_bwd).parameters
    filtered = {key: value for key, value in kwargs.items() if key in supported}
    if filtered:
        return custom_bwd(**filtered)
...
class MSDeformAttnFunction(Function):
    @staticmethod
    @_custom_fwd_compat(cast_inputs=torch.float32, device_type='cuda')
    def forward(
    ...
    @staticmethod
    @once_differentiable
    @_custom_bwd_compat(device_type='cuda')
